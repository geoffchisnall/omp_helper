...
import sys,os,time,subprocess,getpass,csv
...
#Username and Password checking
		...
		while ans:
			...
			19.Detailed list of TASKS
			0.Exit/Quit
		    """)
			try:
				ans=int(raw_input("What would you like to do? "))
			except ValueError:
				os.system('clear')
...
#Get the lastest report on a task
				...
				taskname = subprocess.check_output(taskname, shell=True).strip()
				...
				taskreport = subprocess.check_output(taskreport, shell=True).strip()
				os.system("omp -u %s -w %s --get-report %s --format c402cc3e-b531-11e1-9163-406186ea4fc5 > \"%s\".pdf" % (username,password,taskreport,taskname))
...
#Get the latest report on all tasks
				...
				for task in tasks:
					...
					taskstatus = subprocess.check_output(taskstatus, shell=True)
					taskname=("omp -u %s -w %s -iX '<get_tasks task_id=\"%s\" />' | sed -n '/<name/,/name>/p' | grep \"<name>\"  | sed -e 's/.<name>//' | sed -e 's/<.*//' | awk 'NR==2' | sed \"s/       //\"" %  (username,password,taskid))
					taskname = subprocess.check_output(taskname, shell=True).strip()
					if "New" in taskstatus:
						print ("%s report has not been generated" % (taskname))
					else:
						taskreport="omp -u %s -w %s -iX '<get_tasks task_id=\"%s\" />' | sed -n '/<last_report/,/last_report>/p' | grep \"report id\"  | sed -e 's/<report id=\"//' | sed -e 's/\">.*//' | sed \"s/          //\"" % (username,password,taskid)
						taskreport = subprocess.check_output(taskreport, shell=True).strip()
						os.system("omp -u %s -w %s --get-report %s --format c402cc3e-b531-11e1-9163-406186ea4fc5 > \"%s\".pdf" % (username,password,taskreport,taskname))
						print ("%s report has been generated" % (taskname))
				os.system("rm tasks.xml")
...
#Get the status on a task
				...
				taskstatus = subprocess.check_output(taskstatus, shell=True).strip()
				print "The status of task %s is %s" % (taskid,taskstatus)
...
#Detailed list of TASKS
				...
				xml = xmldoc.getElementsByTagName("xml")[0]
				tasks = xml.getElementsByTagName("task")
				print "TASKNAME", ' '*15, "COMMENT", ' '*15, "STATUS", ' '*20, "TASK ID",' '*23, "SEVERITY",' '*8, "TREND",' '*8, "MODIFICATION TIME"
				for task in tasks:
					taskname = task.getElementsByTagName("name")[1].firstChild.data
					taskcomment = task.getElementsByTagName("comment")[0].firstChild.data
					taskstatus = task.getElementsByTagName("status")[0].firstChild.nodeValue
					tasktrend = task.getElementsByTagName('trend')[0].firstChild
					if tasktrend in [None, '']:
						tasktrend = "empty"
					else:
						tasktrend = task.getElementsByTagName('trend')[0].firstChild.nodeValue
					taskid = task.getAttribute("id")
					taskseverity="omp -u %s -w %s -iX '<get_tasks task_id=\"%s\" />' | sed -n '/<last_report/,/last_report>/p' | grep \"severity\" | grep -Po '(?<=<severity>).*(?=</severity>)'" % (username,password,taskid)
					try:
						taskseverity = subprocess.check_output(taskseverity, shell=True).strip()
					except:
						taskseverity = None
					if taskseverity in ['', None]:
						taskseverity = "empty"
					else:
						pass
					modificationtime = task.getElementsByTagName("modification_time")[0].firstChild.nodeValue
					print taskname, ' '*15, taskcomment, ' '*15, taskstatus, ' '*10, taskid,' '*10, taskseverity, ' '*8, tasktrend, ' '*8, modificationtime
				os.system("rm tasks.xml")
